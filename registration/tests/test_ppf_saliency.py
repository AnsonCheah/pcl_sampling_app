...
unweighted result exactly, non-uniform weights must actually reach the accumulator, and
negative weights must be rejected. Without the first two, no comparison between weighted
and unweighted ablation arms would mean anything.

Run:  python -m pytest registration/tests/test_ppf_saliency.py -q
...
from registration.ppf_saliency import (
    PPFConfig,
    PPFModel,
    SensorProfile,
    alpha_of,
...
def test_key_index_agrees_with_binary_search_and_survives_weighting():
    ...
    model, pts, nrm = _build(_bumpy())
    assert model.key_offsets is not None

    na = model.cfg.n_angle + 1
    keyspace = model.n_dist_bins * na ** 3
    rng = np.random.default_rng(0)
    probe = np.concatenate([model.keys[rng.integers(0, len(model.keys), 400)],
                            rng.integers(0, keyspace, 400)])
    assert np.array_equal(model.key_offsets[probe],
                          np.searchsorted(model.keys, probe, side="left"))
    assert np.array_equal(model.key_offsets[probe + 1],
                          np.searchsorted(model.keys, probe, side="right"))
    assert _build_key_offsets(model.keys, model.n_dist_bins, 400) is None

    w = rng.uniform(0.1, 1.0, model.n_points)
    weighted = model.with_weights(w)
    assert weighted.key_offsets is not None, "with_weights dropped the lookup index"

    unindexed = PPFModel(**{**model.__dict__, "key_offsets": None}).with_weights(w)
    a = match(weighted, pts, nrm, top_k=3)
    b = match(unindexed, pts, nrm, top_k=3)
    assert len(a.poses) == len(b.poses)
    for x, y in zip(a.poses, b.poses):
        assert np.array_equal(x.T, y.T) and x.votes == y.votes


def test_backend_and_match_many_preserve_weighted_results():
    """Neither the GPU path nor the thread pool may change a weighted result.
    ...
    model, pts, nrm = _build(_bumpy())
    w = np.random.default_rng(0).uniform(0.1, 1.0, model.n_points)
    weighted = model.with_weights(w)

    base = match(weighted, pts, nrm, top_k=3)
    assert base.backend == "numpy"

    if cupy_available():
        gpu = match(weighted, pts, nrm, top_k=3, backend="cupy")
        assert gpu.backend == "cupy"
        assert gpu.n_votes == base.n_votes, "GPU cast a different number of weighted votes"
        for a, b in zip(base.poses, gpu.poses):
            assert np.allclose(a.T, b.T, atol=1e-9)
            assert a.votes == pytest.approx(b.votes, rel=1e-9)

    # Unknown backend is still a hard error; a missing GPU is not.
    with pytest.raises(ValueError):
        match(weighted, pts, nrm, backend="cudo")
    assert match(weighted, pts, nrm, backend="auto").backend in ("numpy", "cupy")

    clusters = [(pts, nrm)] * 3
    for got in (match_many(weighted, clusters, workers=1),
                match_many(weighted, clusters, workers=3)):
        assert len(got) == 3
        for r in got:
            assert np.array_equal(r.best.T, base.best.T)
            assert r.best.votes == base.best.votes
...
def test_unknown_backend_is_rejected_but_a_missing_gpu_is_not():
    ...
    model, pts, nrm = _build(_bumpy())
    with pytest.raises(ValueError):
        match(model, pts, nrm, backend="cupy_maybe")
    assert match(model, pts, nrm, backend="numpy").backend == "numpy"
    assert match(model, pts, nrm, backend="auto").backend in ("numpy", "cupy")
