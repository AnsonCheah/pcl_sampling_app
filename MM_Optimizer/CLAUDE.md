...
**Coverage counts orientation, not just position.** `evaluate_config` is called with the tight
angular threshold everywhere except continuous-symmetry parts. It was position-only
(`ang_thresh = 360`) until Sept 2026, which made `angleStep` inert: a finer step costs time and
improves only orientation, so with orientation unscored every sampler drove it to 360. Any
`best_config_*.json` from before that change was tuned without orientation signal and is not
comparable. `REGIME_COVERAGE_GATE` is 0.15 for the same reason -- the gate runs *before*
`angleStep` is tuned, and an N-fold part scores about `1/N` there, so a higher gate would
reject exactly the parts the symmetry search exists to rescue.

**The symmetry search is off unless the bundle says otherwise.** `rotationStrategy` is fixed at
Z (`2.0`) because `pcd_geocenter` puts the ambiguity axis there; `mm_adapter` would default it
to Y (`1.0`), and `angleStep` to `0.0` rather than the intended 360. `angleStep` is only
explored when the PLY header carries `ambiguity_aligned 1` and `ambiguity_fold >= 2`
(`model_sync.symmetry_metadata`). Bundles exported before those keys existed read as
`(1, False)`, so re-export a part with "Recenter to Ambiguity Axis" before expecting the search
to engage. There is no `operationApproach` guard: an initial-commit comment claimed HighSpeed
ignores the rotation, but a live sweep (`test_live_angle_step_takes_effect_per_op_approach`)
shows it applies at all four approaches.
...
uniform `ang_error ~ 179.7deg` at the tight threshold, which looks exactly like unhandled
...
loose regime-gate threshold (`ang_thresh = 360deg`, position-only) and the tight one (5deg). Omit
