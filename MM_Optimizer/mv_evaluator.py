...
Sampler-agnostic base of `tuner.Tuner`. Owns the evaluation contract:
  ...
  - the regime gate (`phase1_regime_gate`)
...
class EvalResult:
    ...
    score_quality:   float = 0.0    # 1 - score/SCORE_WORST_CASE in [0,1]; higher=better
...
class MVEvaluator:
    ...
    def _make_params_dict(self, coarse_params: dict, fine_params: dict,
        ...
        # N x outputNum hypotheses.  Fine matching (candidateTopNum=1) reduces
    ...
    def _default_fine(self) -> dict:
        return {
            ...
            "candidateTopNum":                    1,
            # Symmetry search off, but stated rather than left to mm_adapter, whose defaults
            # are rotationStrategy=1.0 (Y) and angleStep=0.0 -- neither of which is what the
            # tuner means by "off" (that is angleStep=360 about frame Z).
            "rotationStrategy":                   SC.ROTATION_STRATEGY_Z,
            "angleStep":                          float(SC.ANGLE_STEP_NONE),
            "minAngle":                           -180.0,
            "maxAngle":                           180.0,
        }
    ...
    def phase1_regime_gate(self) -> List[dict]:
        ...
        for regime in regimes:
            ...
            scenes = self._sample_scenes(SC.M_FULL)
            # Scored on orientation as well as position, like the study. The gate runs before
            # angleStep is tuned, so a symmetric part is expected to return flipped poses here
            # -- which is why REGIME_COVERAGE_GATE is low rather than why this is loose.
