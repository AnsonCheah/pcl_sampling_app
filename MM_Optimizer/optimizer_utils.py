...
def list_synthetic_plys(path):
    """Return sorted list of sample_N.ply paths.

    *path* may be:
    - A single scene directory (e.g. ``…/scene_00000``) — collects from that
      directory only.
    - A part directory (e.g. ``…/25333MB000``) — aggregates sample_*.ply from
      all ``scene_NNNNN`` subdirectories, giving a larger evaluation pool.

    Paths are returned in ascending scene-folder order, then ascending sample
    index within each folder.
    """
    # Check if path itself contains scene_NNNNN subdirectories
    subdirs = sorted(
        d for d in os.listdir(path)
        if re.match(r'^scene_\d+$', d) and os.path.isdir(os.path.join(path, d))
    )

    if subdirs:
        # Part-level directory: aggregate across all scene subdirs
        all_plys = []
        for scene_subdir in subdirs:
            scene_path = os.path.join(path, scene_subdir)
            plys = sorted(
                [os.path.join(scene_path, f) for f in os.listdir(scene_path)
                 if f.startswith('sample_') and f.endswith('.ply')],
                key=lambda p: int(re.search(r'\d+', os.path.basename(p)).group())
            )
            all_plys.extend(plys)
        return all_plys
    else:
...
def compose_scene(sample_ply_paths, work_dir):
    """Merge one or more sample PLY files into a single scene.ply in work_dir.

    Parameters
    ----------
    sample_ply_paths : list[str]
        Paths to sample_N.ply files (one per instance).
    work_dir : str
        Directory where scene.ply will be written.  Created if absent.

    Returns
    -------
    scene_ply_path : str
        Full path to the written scene.ply — pass this to EasyCreateStringList.strings.
    gt_poses : list[np.ndarray]
        GT pose [x, y, z, qw, qx, qy, qz] for each instance.
    """
    os.makedirs(work_dir, exist_ok=True)
    merged = o3d.geometry.PointCloud()
    gt_poses = []
    for ply_path in sample_ply_paths:
        pcd = o3d.io.read_point_cloud(ply_path)
        merged += pcd
        gt_poses.append(read_gt_pose_from_ply(ply_path, scalar_first=True))
    scene_ply_path = os.path.join(work_dir, "scene.ply")
    o3d.io.write_point_cloud(scene_ply_path, merged)
    return scene_ply_path, gt_poses
...
def cleanup_work_dir(work_dir):
    """Remove a temp scene directory created by compose_scene."""
