...
# ---------------------------------------------------------------------------
# LLM optimizer settings  (llm_optimizer.py)
# ---------------------------------------------------------------------------

LLM_N_ROUNDS                    = 50      # closed-loop refinement rounds per part
LLM_K_SIMILAR                   = 6      # top-K similar parts for ICL retrieval
LLM_MAX_RETRIES                 = 3      # Instructor retry count for structured output
LLM_RETRIEVAL_FALLBACK_THRESHOLD = 0.30  # min similarity score to use ICL examples
LLM_DEFAULT_MODEL               = "phi4-reasoning:latest"
LLM_FALLBACK_MODEL              = "qwen3:8b"

# ---------------------------------------------------------------------------
# TCS (Trajectory Context Summarizer) settings  (tcs.py)
# ---------------------------------------------------------------------------

TCS_MAX_PARETO_SHOWN   = 4      # max non-dominated configs shown in TCS block
TCS_COV_SENSITIVITY    = 0.005  # min |Δcoverage| to report as "up"/"dn" (not "~")
TCS_TIME_SENSITIVITY   = 0.05   # min |Δtime| (s) to report as "up"/"dn"
TCS_HIGH_COV_THRESHOLD = 0.80   # coverage level considered "high" for gap detection
