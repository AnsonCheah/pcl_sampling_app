...
**`pcd_geocenter()` returns a 4x4 transform, not a point.** It is the *inverse* frame transform -- the matrix you APPLY to the cloud to put it in the frame -- so the translation sits in **column 3** (`[:3, 3]`) and row 3 is always `[0, 0, 0, 1]`, as for any homogeneous transform.
...
**The model frame is deterministic but not robust.** `pcd_geocenter` is pure -- same cloud in, bitwise-identical matrix out -- and the whole app path is deterministic (no RNG in `raycast_stage`, `analyse_ambiguity` runs at `seed=0`). But change the voxel size, the view count or the ambiguity config and a *different* ambiguity axis can win: measured on 25333MB000, three sample densities gave folds C4/C1/C2 with the third on an axis 45deg away, moving the frame 60.6deg and 28.3 mm. (Those folds were measured before `fold_pass_fraction` and have not been re-measured; the axis instability they illustrate is unaffected, since fold does not feed the ranking.) Anything pairing a reference cloud with scenes must therefore check they came from the same export -- see `reference_frames_agree()`.
