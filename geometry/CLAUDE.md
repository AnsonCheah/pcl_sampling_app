...
**The model frame is deterministic but not robust.** `pcd_geocenter` is pure -- same cloud in, bitwise-identical matrix out -- and the whole app path is deterministic (no RNG in `raycast_stage`, `analyse_ambiguity` runs at `seed=0`). But change the voxel size, the view count or the ambiguity config and a *different* ambiguity axis can win: measured on 25333MB000, three sample densities gave folds C4/C1/C2 with the third on an axis 45deg away, moving the frame 60.6deg and 28.3 mm. Anything pairing a reference cloud with scenes must therefore check they came from the same export -- see `reference_frames_agree()`.
